...
module Nirum.Targets.TypeScript
    ( TypeScript (..)
    , compilePackage'
    ) where
...
data TypeScript = TypeScript { packageName :: Text }
...
compilePackage' package =
  ...
  where
    ...
    files = [ (toFilename "src" mp, compile (mp, m))
            | (mp, m) <- NMS.toList $ modules package
            ]
    compile :: (ModulePath, Module) -> Either CompileError' Code
    compile (mp, m) =
        Right $ snd $ CB.runBuilder package mp TC.empty (compileModule m)


compileModule :: Module -> CodeBuilder TypeScript ()
compileModule _ = CB.appendCode [compileText|"use strict";|]
